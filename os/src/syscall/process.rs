...
pub fn sys_mmap(_start: usize, _len: usize, _port: usize) -> isize {
    ...
    inner.memory_set.insert_framed_area(VirtAddr(_start),VirtAddr(_start+_len), permission);
    0
}
...
pub fn sys_set_priority(_prio: isize) -> isize {
    ...
    if _prio>=2{
        let current_task: Arc<crate::task::TaskControlBlock>=current_task().unwrap();
        current_task.inner_exclusive_access().prio=_prio as usize;
        _prio
    }else{
        -1
    }
